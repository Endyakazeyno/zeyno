...
global.botname = global.db?.data?.settings?.botName || '𝚭𝚵𝚼𝚴𝚰 𝚩𝚰𝚮'
...
if (!methodCodeQR && !methodCode && !fs.existsSync(`./${authFile}/creds.json`)) {
    do {
    const cyan1 = chalk.hex('#ADD8E6');     // DeepSkyBlue
    const cyan2 = chalk.hex('#E6E6FA');     // DarkTurquoise
    const cyan3 = chalk.hex('#B9B4FF');     // LightSeaGreen
    const green = chalk.hex('#89CFF0');     // Emerald
    const whiteSoft = chalk.hex('#D8BFD8'); // Soft white
    const redSoft = chalk.hex('#DAB6FF');   // Soft red
    ...
    const qr = cyan3(' ⌬') + ' ' + chalk.bold.white('MODALITÀ [1]: collega scansionando il codice QR');
    const codice = cyan3(' ⌬') + ' ' + chalk.bold.white('MODALITÀ [2]: collega con codice a 8 cifre/lettere');
    ...
    const istruzioni = [
        ...
        whiteSoft.italic(''),
        cyan1.italic('                𝚭𝚵𝚼𝚴𝚰 𝐒𝐘𝐒𝐓𝐄𝐌 • 𝐕𝟓.𝟎.𝟎'),
    ];
...
${green.italic('\nSupporto Tecnico: Contatta lo sviluppatore endy lo trovi nei gruppi oppure nel config')}
`);
    }
...
global.zeynoContext = async (conn, jid) => {
    let thumbnail = null
    ...
    try {
        ...
        try {
            thumbnail = fs.readFileSync('./media/image-16.jpg')
        } catch {}
    }
    ...
    if (opzione === '2' || methodCode) {
        ...
        if (!conn.authState.creds.registered) {
            ...
            if (phoneNumber) {
                ...
                phoneNumber = await question(chalk.bgBlack(chalk.bold.hex('#DDA0DD')(`Inserisci il numero di WhatsApp.\n${chalk.bold.hex('B8A9E6')("Esempio: +393501989497")}\n${chalk.bold.hex('#B0E0E6')('─━➤ ')}`)));
            ...
            setTimeout(async () => {
                ...
                console.log(chalk.bold.white(chalk.bgHex('#E0B0FF')('📞 CODICE DI COLLEGAMENTO:')), chalk.bold.white(chalk.hex('##99EEFF')(codeBot)));
            }, 3000);
        }
    }
}
...
async function connectionUpdate(update) {
    ...
    '#A1CAF1', '#CCCCFF', '#99EEFF', '#B0C4DE', '#C3B1E1', '#C9A0DC', '#E6E6FA', '#D8BFD8',
    '#E0B0FF', '#89CFF0', '#A1CAF1', '#E0FFFF', '#E6D9FF', '#B9A7FF'
];
